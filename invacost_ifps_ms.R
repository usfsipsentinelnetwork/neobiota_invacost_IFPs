...
inva.filt <- function(db=invacost,
                      ...
                      gen.ex = T)
  {
    ...
    if (terrestrial) x <- x %>% filter(Environment=="Terrestrial")
    
    if (gen.ex) {
      x <- x  %>% filter(!grepl("Diptera",Order))
      x <- x %>% filter(!(grepl("Hymenoptera",Order) & !grepl("Sirex|Acromyrmex",Genus)))
      x <- x %>% filter(!(grepl("Blattodea",Order) & !grepl("[Tt]ermitidae",Family)))
    }
    
    number_of_records <- x$Reference_ID %>% unique %>% length
    number_of_unique_species <- x$Species %>% unique %>% length
...
world.all.2050.t <- inva.filt(max=2050) ## - all
world.d.2050.t <- inva.filt(damage_type='Damage', #min =1980,
                          max=2050) ### - damage
world.m.2050.t <- inva.filt(damage_type='Management', max=2050) ### - damage

# fungi, arthropods, nematodes - North America

north.america.all.2050.t <- inva.filt(continent="North America", #min =1988,
                                    max=2050) ## - all
north.america.d.2050.t <- inva.filt(continent="North America", damage_type='Damage', #min =1988,
                                  max=2050) ### - damage
north.america.m.2050.t <- inva.filt(continent="North America", damage_type='Management', #min =2000,
                                  max=2050) ### - damage

# fungi, arthropods, nematodes - EU

eu.all.2050.t <- inva.filt(continent="Europe", min=1960, max=2050) ## - all
eu.d.2050.t <- inva.filt(continent="Europe", damage_type='Damage', min=1960, max=2050, nomodel=T) ### - damage
eu.m.2050.t <- inva.filt(continent="Europe", damage_type='Management', min=1960, max=2050) ### - damage
...
rest.of.world.all.2050.t <- inva.filt(elsewhere,max=2050)
rest.of.world.d.2050.t <- #rest.of.world.d.2050.t %>% filter()
  inva.filt(elsewhere, damage_type='Damage',#min=1980,
                                  max=2050)
rest.of.world.m.2050.t <- inva.filt(elsewhere, damage_type='Management',max=2050)

#####

checking <- world.all.2050.t$data %>% select("Order", "Genus","Species","Habitat","Environment","Environment_IAS","Reference_title") %>% distinct %>% arrange(Order, Genus, Species)

# together
to.plot.2050.t <- list(world.all.2050.t,
                       world.d.2050.t,
                       world.m.2050.t,
                       north.america.all.2050.t, 
                       north.america.d.2050.t, 
                       north.america.m.2050.t,
                       eu.all.2050.t,
                       eu.d.2050.t,
                       eu.m.2050.t,
                       rest.of.world.all.2050.t,
                       rest.of.world.d.2050.t,
                       rest.of.world.m.2050.t)

names(to.plot.2050.t)<-
  c(
    "WorldAll",
    "WorldDamage",
    "WorldManagement",
...
invacost %>% filter(Reference_title=="The economic cost of invasive non-native species on Great Britain")

to.plot.2050.t$WorldAll$data$Genus %>% unique
to.plot.2050.t$WorldAll$n.sp
to.plot.2050.t$WorldAll$data %>% filter(Genus == "Diverse/Unspecified") %>% select(Reference_title) %>% unique
# SAVE
cbind(
  to.plot.2050.t$WorldAll$summary$average.cost.per.period[,1:2],
  data.frame(
    cbind(
      to.plot.2050.t$WorldAll$summary$average.cost.per.period[,1:2],
      data.frame(
        WorldAll = to.plot.2050.t$WorldAll$summary$average.cost.per.period$annual_cost,
        WorldDamage = to.plot.2050.t$WorldDamage$summary$average.cost.per.period$annual_cost,
        WorldManagement = to.plot.2050.t$WorldManagement$summary$average.cost.per.period$annual_cost,
        NorthAmericaAll = to.plot.2050.t$NorthAmericaAll$summary$average.cost.per.period$annual_cost,
        NorthAmericaDamage = to.plot.2050.t$NorthAmericaDamage$summary$average.cost.per.period$annual_cost,
        NorthAmericaManagement = to.plot.2050.t$NorthAmericaManagement$summary$average.cost.per.period$annual_cost,
        EuropeAll = to.plot.2050.t$EuropeAll$summary$average.cost.per.period$annual_cost,
        EuropeDamage = to.plot.2050.t$EuropeDamage$summary$average.cost.per.period$annual_cost,
        EuropeManagement = to.plot.2050.t$EuropeManagement$summary$average.cost.per.period$annual_cost,
        AsiaGlobalSAll = to.plot.2050.t$AsiaGlobalSAll$summary$average.cost.per.period$annual_cost,
        AsiaGlobalSDamage = to.plot.2050.t$AsiaGlobalSDamage$summary$average.cost.per.period$annual_cost,
        AsiaGlobalSManagement = to.plot.2050.t$AsiaGlobalSManagement$summary$average.cost.per.period$annual_cost
      )
    )
  )
) %>% write.csv("table3_data.csv", row.names=F)

world.all.2050.t$n
world.d.2050.t$n
world.m.2050.t$n
north.america.all.2050.t$n
north.america.d.2050.t$n
north.america.m.2050.t$n
eu.all.2050.t$n
eu.d.2050.t$n
eu.m.2050.t$n
rest.of.world.all.2050.t$n
rest.of.world.d.2050.t$n
rest.of.world.m.2050.t$n

rest.of.world.all.2050.t$expanded$Genus %>% table
rest.of.world.all.2050.t$expanded$Genus %>% table %>% (function (x) x/sum(x))
north.america.all.2050.t$expanded$Genus %>% table
north.america.all.2050.t$expanded$Genus %>% table %>% (function (x) x/sum(x))

eu.all.2050.t$expanded$Genus %>% table
eu.all.2050.t$expanded$Genus %>% table%>% (function (x) x/sum(x))

(world.all.2050.t$expanded %>%
  select(Genus) %>% table %>% (function (x) x/sum(x)))[c(
    "Hymenoscyphus",
    "Monochamus",
    "Adelges",
    "Lymantria",
...
world.d.2050 <- inva.filt(damage_type='Damage', min =1980,
                          max=2050) ### - damage
world.m.2050 <- inva.filt(damage_type='Management', max=2050) ### - damage

# fungi, arthropods, nematodes - North America

north.america.all.2050 <- inva.filt(continent="North America", min =1988,
                                    max=2050) ## - all
north.america.d.2050 <- inva.filt(continent="North America", damage_type='Damage', min =1988,
                                  max=2050) ### - damage
north.america.m.2050 <- inva.filt(continent="North America", damage_type='Management', min =2000,
                                  max=2050) ### - damage
...
rest.of.world.d.2050 <- inva.filt(elsewhere, damage_type='Damage',min=1980,
                                  max=2050)
...
to.plot.2050.4$WorldAll$trend
to.plot.2050.4$EuropeAll$trend
to.plot.2050.4$NorthAmericaAll$trend
to.plot.2050.4$AsiaGlobalSAll$trend
